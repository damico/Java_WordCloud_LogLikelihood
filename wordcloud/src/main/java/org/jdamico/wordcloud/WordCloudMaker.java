...
import org.jdamico.wordcloud.RotatedLabel.Direction;
import org.mcavallo.opencloud.Cloud;
import org.mcavallo.opencloud.Tag;

/**
 * create java word clouds maker using two arrays (first column containing
 * keywords, second containing size (freq or LLH). The method uses JAMESLIC
 * method to rotate java labels, which I reused to give the word cloud a better
 * lookd and feel
 * 
 * @author elhaj
 *
...
public class WordCloudMaker {
	...
	/**
	 * creates a word cloud using random colours and renders the size which I
	 * advise to tune down a bit if you expect larger values in the double
	 * array.
	 * 
	 * @throws IOException
	 */
	protected void initUI() throws IOException {
		...
		for (Tag tag : cloud.tags()) {
			...
			label.setToolTipText(
					"<html><p>Value = " + f.format(size[Arrays.asList(words).indexOf(tag.getName())]) + "</p></html>");
		...
		/*
		 * optional JButton to refresh the frame to recolour the words. JButton
		 * reload = new JButton("Reload"); reload.addActionListener(new
	...
	/**
	 * main method needed to run and create word clouds, a simple method that
	 * takes an array of word strings and another array for those words weight
	 * (size, frequency or LLH) values, which gives a better representation than
	 * the random font size word clouds online. The word cloud does not repeat
	 * words which makes it very efficient.
	 * 
	 * @param wordsArray
	 * @param sizeArray
	 * @param frameTitle
